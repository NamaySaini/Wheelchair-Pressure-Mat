...
export class BackendClient {
  ...
  async startSession() {
    ...
    return this.request<SessionStartResponse>('/sessions', { method: 'POST', requiresAuth: true });
  }
  ...
  async listSessions(period: PressurePeriod = 'week') {
    ...
    return this.request<SessionRow[]>(`/sessions?period=${period}`, { requiresAuth: true });
  }
  ...
  async getTracker(period: TrackerPeriod, anchor: string) {
    ...
    return this.request<TrackerResponse>(`/tracker?${params.toString()}`, { requiresAuth: true });
  }
  ...
  async postReading(reading: ReadingPayload) {
    ...
    return this.request<{ success: true }>('/readings', {
      method: 'POST',
      body: reading,
      requiresAuth: true,
    });
  ...
  async getChatHistory() {
    ...
    return this.request<ChatMessage[]>('/chat/history', { requiresAuth: true });
  }
}

export const backendClient = new BackendClient(async () => {
